...
uint16_t ps1::nodevice_fetch16(void* device, mem_addr_t offset) {
    DEBUG_CODE(logger::push("16 bit fetching from nodevice", logger::type_t::warning));

    return 0;
}


uint8_t ps1::nodevice_fetch8(void* device, mem_addr_t offset) {
    DEBUG_CODE(logger::push("8 bit fetching from nodevice", logger::type_t::warning));
